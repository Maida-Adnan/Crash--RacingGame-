...
.collision_detected:
    ; FIRST: Move player to the collision lane
    mov ax, [player_row]
    mov bx, [player_col]
    call clear_car_at
    dec byte [player_lane]
    ...
    ; FINALLY: Draw spark effect and set game over
    call draw_spark
...
.print_press:
    lodsb
    cmp al, 0
    je .print_dev1_setup    ; <-- CHANGED THIS LINE
    mov [es:di], ax
    add di, 2
    jmp .print_press

.print_dev1_setup:          ; <-- NEW CODE STARTS HERE
    mov ax, 7
    mov bx, 28
    call xy_to_di
    mov si, developer_1
...
.print_dev2:
    lodsb
    cmp al, 0
    je .done               ; <-- NEW CODE ENDS HERE
    mov [es:di], ax
    add di, 2
    jmp .print_dev2
    
.done:                     ; <-- KEEP THIS LABEL
    popa
    ret
...
mov word [music_index], 0        ; ADD
    mov word [music_tick_counter], 0 ; ADD
...
.cooldown_zero:
    ...
    mov byte [end_reason], 1        ; 1 = ran out of fuel
    mov byte [show_gameover_popup], 1
    ret                             ; Exit update immediately
.fuel_ok:
...
Start:
    call init_game
    call hook_keyboard
	   call hook_timer          ; ADD - Start music proce
    call clrscr
    call game_loop
	 call stop_sound                  ; ADD
  call unhook_timer       ; ADD - Stop music process 
